...
/////// Diffuse:
                ...
                float4 node_3025 = _Time;
                float node_133 = pow((abs((frac((i.uv0+node_3025.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
...
/////// Diffuse:
                float NdotL = dot( normalDirection, lightDirection );
                float4 node_4454 = _Time;
                float node_133 = pow((abs((frac((i.uv0+node_4454.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
                float3 w = float3(node_133,node_133,node_133)*0.5; // Light wrapping
                float3 NdotLWrap = NdotL * ( 1.0 - w );
                float3 forwardLight = max(float3(0.0,0.0,0.0), NdotLWrap + w );
                float3 backLight = max(float3(0.0,0.0,0.0), -NdotLWrap + w ) * float3(node_133,node_133,node_133);
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float4 node_7923 = _Time;
                float node_133 = pow((abs((frac((i.uv0+node_7923.g*float2(0.25,0)).r)-0.5))*2.0),_BulgeShape); // Panning gradient
